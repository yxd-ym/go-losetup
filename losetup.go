...
func GetFree() (*Device, error) {
	ctrl, err := os.OpenFile(LoopControlPath, os.O_RDWR, 0660)
	if err != nil {
		return nil, fmt.Errorf("could not open %v: %w", LoopControlPath, err)
	}
	defer ctrl.Close()
	dev, _, errno := unix.Syscall(unix.SYS_IOCTL, ctrl.Fd(), CtlGetFree, 0)
	if errno != 0 {
		return nil, fmt.Errorf("could not get free device (err: %d): %w", errno, errno)
	}
...
func Attach(backingFile string, offset uint64, ro bool) (*Device, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not open loop device: %w", err)
	}
	...
	if errno == 0 {
		...
		if err := setInfo(loopFile.Fd(), info); err != nil {
			...
			return nil, fmt.Errorf("could not set info: %w", err)
		}
		err := setDIO(loopFile.Fd())
		return dev, err
	} else {
		return nil, errno
	}
